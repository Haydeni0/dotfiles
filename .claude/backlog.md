...
- #2 [2026-09-13] herdr remote session intermittently loses state on ssh in - FIXED and verified.
  ...
  - Remaining follow-ups: old `sessions/main/` dir left on cluster home (archive of pre-fix layout; delete when confident). Upstream issue draft at `~/.claude/jobs/98ce5ce0/tmp/herdr-nfs-socket-race-issue.md` - NOT yet filed with herdrdev/herdr (user to approve publishing). Option B rejected, `--remote` federation noted as future alternative for "one session from many nodes".
